...
The goal of MargIntTmle is to estimate marginal interaction effects using g-computation, IPTW or TMLE.
...
Several quantities of interest are than calculated using the `estim.int.effects` function.
...
The results can be presented in a table following Know and VanderWeele recommendations (2012). The out.table object contains the table and the interaction.effects object contains the additive, multiplicative interaction effects and the RERI.

The interaction table can be rendered using the `kableExtra` package.
```{r table_interaction, eval = FALSE}
